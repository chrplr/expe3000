...
void run_experiment(Config *cfg, Experiment *exp, Resource *resources, 
    ...
    bool run = true; SDL_Event ev; Uint64 st_ticks = SDL_GetTicks();
    ...
    while (run) {
        ...
        while (SDL_PollEvent(&ev)) {
            if (ev.type == SDL_EVENT_QUIT) run = false;
                ...
                if (ev.key.key == SDLK_ESCAPE) run = false;
                else log_event(log, ct, "RESPONSE", SDL_GetKeyName(ev.key.key));
            }
        }
            ...
            if ((s->type == STIM_IMAGE || s->type == STIM_TEXT) && resources[cs].texture) {
                ...
                for (int j = 0; j < MAX_ACTIVE_SOUNDS; j++) {
                    if (!mx->slots[j].active) {
                        ...
                        log_event(log, ct, "SOUND_ONSET", s->file_path);
        ...
        if (avi != -1 && ct >= vet) {
            log_event(log, ct, exp->stimuli[avi].type == STIM_IMAGE ? "IMAGE_OFFSET" : "TEXT_OFFSET", exp->stimuli[avi].file_path);
        ...
        } else if (cfg->use_fixation) draw_fixation_cross(rend, cfg->screen_w, cfg->screen_h);
        SDL_RenderPresent(rend);

        if (trig) {
            Uint64 ot = SDL_GetTicks() - st_ticks;
            log_event(log, ot, exp->stimuli[tidx].type == STIM_IMAGE ? "IMAGE_ONSET" : "TEXT_ONSET", exp->stimuli[tidx].file_path);
